...
Server::Server(std::string name, std::sting addr, std::string config_path)
    ...
    update_channel();
    kv_server = new KeyValueStoreServer(
        identity = identity,
        raft_clients = raft_clients);  // assign port name, raft_clients
...
Server::run() {
    ...
    builder.RegisterService(&kv_server) std::unique_ptr<Server> server(
...
bool Server::read_server_config(std::string config_path,
                                shared_ptr<KeyValueStore>& server_config) {
